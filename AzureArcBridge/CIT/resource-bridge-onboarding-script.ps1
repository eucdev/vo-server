...
$supportMsg = "`nPlease refer to the Troubleshooting guide https://aka.ms/arcscvmmtsg for self-debugging. If the issue persists, kindly create a support ticket for Azure Arc enabled SCVMM from Azure portal for further assistance."
$deployKVATimeoutMsg = "`nPlease refer to the Troubleshooting guide https://aka.ms/arcscvmmtsg for self-debugging. If the issue persists, kindly create a support ticket for Azure Arc enabled SCVMM from Azure portal for further assistance.`nIn case of DeployKvaTimeoutError please run the following steps to collect the logs and upload them to the support ticket. `n`t`"az arcappliance logs scvmm [Appliance_VM_IP]`"`nwhere Appliance_VM_IP is the IP of the Appliance VM Created in SCVMM"
...
try {
    ...
    if ($invokeApplianceRun) {
        ...
        az arcappliance run scvmm --tags 'Datacenter=CIT' 'Environmnet=Production' @forceParam --subscription $applianceSubscriptionId --resource-group $applianceResourceGroupName --name $applianceName --location $location --address $scvmmDetails[$scFqdnKey] --port $scvmmDetails[$scPortKey] --username $scvmmDetails[$scUsernameKey] --password $scvmmDetails[$scPasswordKey]
    } else {
    ...
    az customlocation create --debug --tags 'Datacenter=CIT' 'Environmnet=Production' --subscription $customLocationSubscriptionId --resource-group $customLocationResourceGroupName --name $customLocationName --location $location --namespace $customLocationNamespace --host-resource-id $applianceId --cluster-extension-ids $clusterExtensionId 2>> $logFile
